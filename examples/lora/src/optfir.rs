...
fn low_pass(
    ...
    // by
    // default
    taps = pm_remez(n + nextra_taps, &fo, &ao, &w, "bandpass", 16);
    taps
}
...
fn passband_ripple_to_dev(ripple_db: f64) -> f64 {
    ...
    // ""
    return (10.0.powf(ripple_db / 20.) - 1.) / (10.0.powf(ripple_db / 20.) + 1.);
}
//
// //  ----------------------------------------------------------------
//
