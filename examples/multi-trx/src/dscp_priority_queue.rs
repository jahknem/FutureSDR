...
impl<T1, T2> BoundedDiscretePriorityQueue<'_, T1, T2> where T1: std::fmt::Debug, T2: Eq + Hash + Copy + Display + Binary + Ord{
    pub fn new(new_max_size: usize, new_priority_values: &[T2]) -> BoundedDiscretePriorityQueue<T1, T2> {
    ...
    pub fn push_back(&mut self, value: T1, priority: T2) {
        match self.priority_index_map.get(&priority) {
            Some(insert_index_ref) => {
                ...
                debug!("inserted new frame at position {}, queue len {}", insert_index, self.data.len());
            ...
            None => warn!("Packet contained an invalid DSCP value: {:#8b}. DROPPING. full packet: {:?}", priority, value),
        }
    }
}
